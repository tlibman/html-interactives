// Epsilon-Delta Limits Learning Tool
// For different functions at the point x=1

let canvas;
let epsilon = 0.5;
let delta = 0.5 / 4; // Fixed at epsilon/4 for continuous, variable for others
let selectedFunction = 'continuous'; // 'continuous', 'discontinuous', or 'removable'
...
function drawTitle() {
    ...
    } else if (selectedFunction === 'discontinuous') {
        text("f(x) = x² + step function", width/2, 25);
    } else {
        text("f(x) = x² except f(1)=1.5", width/2, 25);
    }
    textSize(14);
...
function drawFunction() {
    ...
    if (selectedFunction === 'continuous') {
    ...
    } else if (selectedFunction === 'removable') {
        // Draw removable discontinuity function
        // Left side: x < 1
        beginShape();
        for (let x = xMin; x < 1; x += 0.01) {
            let y = evaluateFunction(x);
        ...
        // Draw open circle at the limit point (1, 1)
        let limitX = mapX(1);
        let limitY = mapY(1);
...
function evaluateFunction(x) {
    if (selectedFunction === 'continuous') {
        ...
        // f(x) = x² + step function (limit does not exist at x=1)
        if (x < 1) {
        ...
        // f(x) = x² except f(1)=1.5 (limit exists but doesn't equal f(1))
        if (x === 1) {
            return 1.5; // f(1) = 1.5
        } else {
            return x * x; // f(x) = x² for all other x
        }
    }
}

function drawEpsilonStrip() {
    // Horizontal strip around the limit value at x = 1
    let centerY = getLimitValue();
...
function getLimitValue() {
    // Return the limit value at x=1 for each function
    if (selectedFunction === 'continuous') {
        return 1; // lim(x→1) x² = 1
    } else if (selectedFunction === 'discontinuous') {
        return 1; // This is the left-hand limit, but the limit doesn't exist
    } else if (selectedFunction === 'removable') {
        return 1; // lim(x→1) x² = 1 (even though f(1) = 1.5)
    }
}
...
function updateUIForFunction() {
    ...
    const epsilonSection = epsilonSlider.parentElement.parentElement;
    ...
    if (selectedFunction === 'continuous') {
        ...
        epsilonSection.querySelector('.value-display').textContent = 'Adjust to change the height of the horizontal strip';
        deltaSection.querySelector('h3').textContent = 'Delta (δ) Information';
        deltaSection.querySelector('.value-display').textContent = 'Fixed at ε/4 for this function';
    } else if (selectedFunction === 'discontinuous') {
        ...
        epsilonSection.querySelector('.value-display').textContent = 'Fixed at 0.25 for this example';
        ...
        // For removable discontinuity: epsilon is variable, delta is fixed at epsilon/4
        epsilonSlider.disabled = false;
        deltaSlider.disabled = true;
        epsilonSection.querySelector('.value-display').textContent = 'Adjust to change the height of the horizontal strip';
        deltaSection.querySelector('h3').textContent = 'Delta (δ) Information';
...
function updateDelta() {
    if (selectedFunction === 'continuous' || selectedFunction === 'removable') {
